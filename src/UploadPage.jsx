...
function UploadPage() {
  ...
  const handleGoToSecondPage = () => {
    ...
    const featuresSelected = selectedFeatures.join(", ");
    const targetsSelected = selectedTargets.join(", ");

    const fileContent = `Dataset name: ${datasetName}\nFeatures: ${featuresSelected}\nTarget: ${targetsSelected}`;

    // Save the file content to localStorage
    localStorage.setItem("dataset_info.txt", fileContent);

    // Log the content to console (optional)
    console.log("Dataset information saved:", fileContent);
    ...
    fetch(`http://10.130.2.83:8000/api/file`, {
      method: "POST",
      body: formData,
    })
      .then((response) => {
